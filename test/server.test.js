...
describe('DELETE /todos/id', () => {
  ...
  it("should not remove a todo when unauthenticated", done => {
...
describe('PATCH /todos/:id', () => {
  ...
  it("should not update other users' todos", done => {
    // grab id of first item
    const hexId = todos[0]._id.toHexString()
    const text = "First test todo updated"
...
describe('DELETE /users/me/token', () => {
  it('should remove auth token on logout', (done) => {
    ...
    request(app)
      .delete('/users/me/token')
      // set token = users[1].tokens.token
      .set('x-auth', users[0].tokens[0].token)
      // 200
      .expect(200)
      // Find user, veriyf that token array has length 0
      .end((err, res) => {
        if (err) return done(err)

        Users.findById({ _id: users[0]._id })
          .then(user => {
